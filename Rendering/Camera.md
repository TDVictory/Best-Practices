...
## Geometry（几何学）
将你场景内的物体几何复杂度最小化是十分重要的，不然Unity就要给显卡推送大量的顶点数据。一个低端移动设备大约能承受20万静态三角面。当然，这也取决于你的物体时运动的还是静止的。

| 平台（Platform） | 静态几何体（Static Geometry）（/百万三角面） | 运动表面网格（Animated Skinned Meshes）（/百万三角面） | 
| - | :-: | -: | 
| 低端移动设备 | 0.2| 0.05 | 
| 高端移动设备 | 1-5 | 0.5-1 | 

在Unity，用少量的高面物体的渲染效果要比用大量的低面物体的渲染效果好。

将你看不到的面从几何体上移除，不渲染任何玩家看不到的东西。例如，你看不到橱柜靠墙的那一面，所以橱柜模型在背面不需要任何面。

尽可能地精简面数，根据所使用的平台（特别是移动端），使用高分辨率的纹理来填充细节从而弥补低面模型带来的影响，除此之外还有视差贴图和曲面细分技术。记得定期做性能测试，确认是否对显示效果有影响，或者是否适宜当前的目标平台。

通过将尽可能多的细节烘培进贴图纹理来降低像素复杂度（每个像素的计算量）。举个例子，将镜面高光烘培进纹理来避免在片元着色器上计算高光。

## Level of Detail（LOD）
Level Of Detail（LOD）渲染允许你能够在物体离摄像机越远时减少其渲染的三角面。只要物体不是在同一时间全部都靠近摄像机，LOD可以通过增加一个LOD组件来降低硬件上的读取并且改善渲染效果。它也会根据与摄像机的距离划分的距离组提供多个低细节的网格。总共有8个LOD等级。类似Simplygon这类的工具可以自动处理LOD的大部分资产准备工作（Tools like Simplygon can automate much of the Asset preparation process for LOD.）。

![](/Image/Rendering/lod0.png)

![](/Image/Rendering/lod1.png)

### Static Scenes（静态场景）
当你设置了一个静止的相机，也即你的人物时无法移动的（就像在一些VR内体验的那样），最好使用一个能根据当前距离展现出正确的细节的网格而不是对每个物体都设置大量的网格数据。在纹理贴图的运用上也是同理——你应该使用一个合适的贴图分辨率而不是在运行时进行贴图的分级细化。应用正确的细节也将节省大量的磁盘空间和一部分运行内存。

### High-Quality LODs（高质量LOD）
如果你可以满足内存需求，你就可以使用网格合并并且对合并结果进行LOD划分。举个例子，一个书柜中包含一个离相机特别近特别的部分，但是你把它们都混合进一个单独的网格中并按照距离进行了细节分级（LOD）。那么即便你意见优化了几何体，材质，着色器，也依然会花费时间和性能来创建与保持高质量的LOD。

### Runtime Mesh Combination（运行时的网格合并）
